// app/booking-history/view/page.tsx
"use client";
...
import {
  ...
  BookingHistoryBasicDetails,
  BookingHistoryRequestParams,
...
import {
  BOOKING_HISTORY_PAGE_URL,
  BOOKING_HISTORY_VIEW_PAGE_URL,
} from "@/utils/urls";
import BookingHistoryCard from "@/components/booking-history-components/view-booking-history-components/BookingHistoryCard";
import BookingHistoryListCard from "@/components/booking-history-components/view-booking-history-components/BookingHistoryListCard";
import {
  bookingHistoryFiltersToUrlParams,
  bookingHistoryUrlParamsToFilters,
} from "@/utils/urlParameterFunctions";
import { FilterField } from "@/types/filter-types";
import { BOOKING_HISTORY_VIEW_SORTING_OPTIONS } from "@/data/sorting-options";
import { BOOKING_HISTORY_VIEW_HOME_BREADCRUMB_DATA } from "@/data/breadcrumb-data";

const BookingHistoryViewContent = () => {
  ...
  const [requestParams, setRequestParams] =
    useState<BookingHistoryRequestParams | null>(null);
  const [requestParamsLoading, setRequestParamsLoading] = useState(true);

  const [filters, setFilters] = useState<BookingHistoryFilterParams>(() =>
    bookingHistoryUrlParamsToFilters(searchParams || new URLSearchParams()),
  );

  const [bookings, setBookings] = useState<BookingHistoryBasicDetails[]>([]);
  ...
  useEffect(() => {
    const fetchRequestParams = async () => {
      try {
        ...
        const response =
          await BookingHistoryService.getBookingHistoryRequestParams();
        if (response.code === 200 && response.data) {
          setRequestParams(response.data);
        }
      } catch (error) {
        console.error("Error fetching booking history request params:", error);
      } finally {
  ...
  const getSortLabel = (sortBy: string): string => {
    const option = BOOKING_HISTORY_VIEW_SORTING_OPTIONS.find(
  ...
  const updateURL = useCallback(
    (newFilters: BookingHistoryFilterParams) => {
      const params = bookingHistoryFiltersToUrlParams(newFilters);
      const queryString = params.toString();
      const newURL = queryString
        ? `${BOOKING_HISTORY_VIEW_PAGE_URL}?${queryString}`
        : BOOKING_HISTORY_VIEW_PAGE_URL;
  ...
  const fetchBookings = useCallback(
    async (currentFilters: BookingHistoryFilterParams) => {
      setLoading(true);
      try {
        const response =
          await BookingHistoryService.getBookingHistory(currentFilters);

        if (response.code === 200 && response.data) {
          setBookings(response.data.bookingHistoryBasicDetailsResponses || []);
          setTotalItems(response.data.bookingHistoryCount || 0);
        } else {
          setBookings([]);
          setTotalItems(0);
        }
  ...
  useEffect(() => {
    const initialFilters = bookingHistoryUrlParamsToFilters(
  ...
  useEffect(() => {
    if (!isInitialLoad) {
      const urlFilters = bookingHistoryUrlParamsToFilters(
        searchParams || new URLSearchParams(),
      );
  ...
  const handleReset = () => {
    const resetFilters: BookingHistoryFilterParams = {
      name: null,
      minPrice: null,
      maxPrice: null,
  ...
  const handleRemoveSort = () => {
    const updatedFilters: BookingHistoryFilterParams = {
      ...filters,
      sortBy: null,
      sortDirection: "ASC",
      pageNumber: 1,
  ...
  if (requestParamsLoading) {
    return (
      <CommonLoading
        ...
        subMessage="Please wait while we fetch booking history and filters"
        size="md"
      />
    );
  }
    ...
    >
      ...
      >
        <div className="mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <PageHeader
            title="Booking History View"
            description="View and manage complete booking history and records"
            breadcrumbItems={BOOKING_HISTORY_VIEW_HOME_BREADCRUMB_DATA}
          />
        </div>
      </div>
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <FilterPanel
            ...
            showPageSize={true}
            showSorting={true}
            sortOptions={BOOKING_HISTORY_VIEW_SORTING_OPTIONS}
            sortBy={filters.sortBy || ""}
            sortDirection={(filters.sortDirection as "ASC" | "DESC") || "ASC"}
            title="Filter Booking History"
        ...
        {loading && (
          <CommonLoading
            message="Loading booking history..."
            subMessage="Fetching booking records and history"
            size="lg"
          />
        )}
...
const BookingHistoryViewPage = () => {
  ...
  return (
    ...
    >
      <BookingHistoryViewContent />
    </Suspense>
  );
};

export default BookingHistoryViewPage;
